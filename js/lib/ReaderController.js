           ...
           UL.url_load(your_url, function (html_text) {
             if (html_text) { // the query did not fail to return a non-empty text
               ...
               if (!extract_promise) {
                 ...
                 dfr.reject(new DS.Error("<p> ERROR : nothing to display </p><p> Possible cause : no important paragraph could be identified </p>"),
                            null);
                 return;
               }
               ...
               extract_promise
                  .done(function (err, html_text) {
                          logWrite(DBG.TAG.DEBUG, "extract relevant resolved!");
